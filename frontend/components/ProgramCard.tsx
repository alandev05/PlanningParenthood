...
export default function ProgramCard({ program, onPress }:{program: Program; onPress: ()=>void}){
  return (
    <TouchableOpacity style={styles.card} onPress={onPress}>
      <View style={styles.row}>
        <View style={styles.titleWrap}>
          <Text style={styles.title}>{program.title}</Text>
          <Text style={styles.why} numberOfLines={2}>{program.why}</Text>
        </View>
      ...
      <View style={styles.rowBottom}>
        <Text style={styles.age}>Ages {program.ageRange[0]}–{program.ageRange[1]}</Text>
...
const styles = StyleSheet.create({
  card: {
    ...
    marginHorizontal: 16,
    shadowColor: '#000',
    shadowOffs    et: { width: 0, height: 6 },
    shadowOpacity: 0.06,
    shadowRadius: 12,
    elevation: 3,
  },
  ...
  rowBottom: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 12 },
